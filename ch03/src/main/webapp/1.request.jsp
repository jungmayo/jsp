...
<title>1.request</title>
	<%--
		...
		GET 요청
		 - GET 요청 방식은 HTTP 기본 요청방식
		 - GET은 서버에 있는 자원을 요청하는 방식
		 - GET요청으로 데이터를 전송할때는 Query String(parameter)로 요청주소에 노출되어 전송된다.
		 
		POST 요청
		 - POST는 사용자가 입력한 값을 서버로 전송하기 위한 요청방식
		 - 입력한 전송데이터는 주소에 노출되지 않고 요청 메세지 바디에 삽입되어 전송
		 - GET방식에 비해서 보안에 유리하고 대량의 데이터 전송에 적합하다
	--%>
</head>
<body>
	...
	<h4>확인문제</h4>
	<form action="./proc/result3.jsp" method="Get">
		<table border="1">
		<tr>
		<td>아이디</td>
		...
		<td>성별</td>
		<td>
		<label><input type="radio" name="gender" value="남자"/>남</label>>
		<label><input type="radio" name="gender" value="여자"/>여</label>>
		</td>
		</tr>
